...
// GET /api/admin/profile/:id - Fetch admin profile
router.get('/profile/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const admin = await Admin.findById(id).select('_id username email role createdAt');
    if (!admin) {
...
// PUT /api/admin/profile/:id - Update admin profile (username/email/password)
router.put('/profile/:id', async (req, res) => {
  try {
    ...
    if (mongoose.connection.readyState !== 1) {
      return res.status(503).json({ message: 'Database unavailable. Profile changes were not saved.' });
    }
    ...
    const updatePayload = { username: normalizedUsername, email: normalizedEmail };
    if (password) {
      if (String(password).length < 8) {
        return res.status(400).json({ message: 'Password length at least 8 characters.' });
      }
      updatePayload.password = String(password);
    }

    const updatedAdmin = await Admin.findByIdAndUpdate(id, updatePayload, { new: true, runValidators: true });
...
// DELETE /api/admin/users/:id - Delete user (community or researcher)
router.delete('/users/:id', async (req, res) => {
  try {
    ...
    if (!deletedUser) {
      deletedUser = await Researcher.findByIdAndDelete(id);
      userType = 'researcher';
    }
    ...
    // Optional: delete user's reports too
