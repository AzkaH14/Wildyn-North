...
router.get('/', async (req, res) => {
  try {
    const reports = await Report.find(
      { isSpam: { $ne: true }, deletedBy: null },
      'image specieName healthStatus location timestamp username userId comments pinnedComment researcherStatus markedBy markedAt createdAt'
    )
      .sort({ createdAt: -1 })
      .limit(100)
      .lean();

    const usernames = [...new Set(reports.map((r) => r.username).filter(Boolean))];
    let profileImageByUsername = {};

    if (usernames.length) {
      const [communityUsers, researchers] = await Promise.all([
        User.find({ username: { $in: usernames } }).select('username profileImage').lean(),
        Researcher.find({ username: { $in: usernames } }).select('username profileImage').lean(),
      ]);

      profileImageByUsername = [...communityUsers, ...researchers].reduce((acc, user) => {
        if (user?.username && user?.profileImage) acc[user.username] = user.profileImage;
        return acc;
      }, {});
    }

    const reportsWithProfileImages = reports.map((report) => ({
      ...report,
      reporterProfileImage: profileImageByUsername[report.username] || null,
    }));

    console.log(`📊 Returning ${reports.length} reports`);
    res.json(reportsWithProfileImages);
  } catch (error) {
...
router.post('/:id/pin', async (req, res) => {
  try {
    ...
    const incomingComment = req.body?.comment || {};
    const normalizedPinnedComment = {
      text: incomingComment.text || '',
      user: incomingComment.user || 'Anonymous',
      userId: incomingComment.userId || 'anonymous',
      timestamp: incomingComment.timestamp ? new Date(incomingComment.timestamp) : new Date(),
      pinned: true,
    };

    if (!normalizedPinnedComment.text.trim()) {
      return res.status(400).json({ message: 'Valid comment text is required to pin' });
    }

    report.pinnedComment = normalizedPinnedComment;
    await report.save();
