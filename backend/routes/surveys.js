...
router.post('/:id/respond', async (req, res) => {
  try {
    ...
    if (survey.status !== 'published') {
      return res.status(400).json({ message: 'This survey is not open for responses' });
    }

    const uid = userId || '';
    if (uid && String(survey.researcherId) === String(uid)) {
      return res.status(403).json({ message: 'Researchers cannot submit responses to their own surveys' });
    }
