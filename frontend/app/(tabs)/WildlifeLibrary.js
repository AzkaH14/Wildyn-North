      ...
      >
        ...
        <View style={styles.cardContent}>
          ...
          <View style={styles.viewMore}>
            ...
            <Ionicons name="arrow-forward" size={16} color="#236550" />
          </View>
        </View>
      </TouchableOpacity>
      <TouchableOpacity 
...
export default function WildlifeLibrary() {
  ...
  // Load wildlife data from backend database
  const loadWildlifeData = async () => {
    try {
      setLoading(true);
      const response = await fetch(`${API_URL}/api/wildlife`);
      if (!response.ok) {
        throw new Error(`Failed to fetch wildlife cards (${response.status})`);
      }
    ...
    Alert.alert(
      ...
      [
        ...
        },
        {
          ...
          onPress: async () => {
            try {
              const response = await fetch(`${API_URL}/api/wildlife/${speciesId}`, {
                method: 'DELETE',
              });
              if (!response.ok) {
                throw new Error(`Failed to delete wildlife card (${response.status})`);
              }
              loadWildlifeData();
            } catch (error) {
