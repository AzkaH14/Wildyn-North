...
/* ─── SVG Bar Chart ─── */
function MiniBarChart({ data, color = '#1a5f3a' }) {
...
/* ─── Circular Progress ─── */
function CircleProgress({ value = 0, max = 100, size = 80, color = '#1a5f3a', label }) {
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 12 }}>
        ...
        {card.trend !== undefined && (
          <div style={{
            ...
            color: card.trend >= 0 ? '#1a5f3a' : '#ef4444',
            background: card.trend >= 0 ? '#e9f3ed' : '#fee2e2',
...
function HealthDot({ label, status }) {
  const colors = { ok: '#1a5f3a', warn: '#f59e0b', error: '#ef4444' };
  return (
...
export default function Dashboard({ admin, onNavigate }) {
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: 320, gap: 16 }}>
        <div style={{ position: 'relative', width: 52, height: 52 }}>
          <div style={{
            ...
            border: '3px solid #e5ede9', borderTopColor: '#1a5f3a',
            animation: 'spin 0.8s linear infinite',
          }} />
          <div style={{
            position: 'absolute', inset: 10,
            borderRadius: '50%', border: '2px solid #cfe2d5',
            borderTopColor: '#1a5f3a', opacity: 0.5,
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: 320, gap: 16 }}>
        ...
        <button onClick={fetchStats} style={{
          padding: '10px 24px', background: '#1a5f3a', color: 'white',
          border: 'none', borderRadius: 10, fontWeight: 700,
          cursor: 'pointer', fontFamily: 'DM Sans, sans-serif', fontSize: 14,
          boxShadow: '0 4px 12px rgba(26,95,58,0.25)',
        }}>Retry</button>
      </div>
    );
  }
  ...
  const statCards = [
    ...
    { label: 'Normal Reports',       value: normal,  icon: CheckCircle2, border: '#1a5f3a', bgLight: '#e9f3ed', trend: 18 },
  ...
  const activityFeed = [
    ...
    { icon: CheckCircle2, color: '#1a5f3a', bg: '#e9f3ed', text: 'Researcher profile verified', time: '1 hr ago' },
  ...
  const quickActions = [
    ...
    { label: 'Manage Users',      icon: Users,     tab: 'users',       color: '#1a5f3a', bg: '#e9f3ed' },
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 28 }}>
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 12px',
            background: '#e9f3ed', borderRadius: 99, border: '1px solid #cfe2d5' }}>
            <span style={{ width: 7, height: 7, borderRadius: '50%', background: '#1a5f3a',
              boxShadow: '0 0 0 3px #cfe2d5', animation: 'pulse 2s infinite' }} />
            <span style={{ fontSize: 12, color: '#155131', fontWeight: 600 }}>Live</span>
          </div>
          <button onClick={fetchStats} style={{
        ...
        <div style={{
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              {['reports', 'researchers'].map(tab => (
                <button key={tab} onClick={() => setActiveChart(tab)} style={{
                  ...
                  background: activeChart === tab ? '#1a5f3a' : '#f0f5f2',
          ...
          <div style={{ display: 'flex', gap: 20, marginTop: 16, paddingTop: 16, borderTop: '1px solid #f0f5f2' }}>
            {(activeChart === 'reports' ? [
              ...
              { label: 'Normal Rate',  value: total ? `${Math.round(normal / total * 100)}%` : '0%', color: '#1a5f3a' },
            ] : [
              { label: 'Verify Rate', value: totalResearchers ? `${Math.round(verified / totalResearchers * 100)}%` : '0%', color: '#1a5f3a' },
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 260px', gap: 20 }}>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
            <h3 style={{ fontFamily: "'DM Serif Display', serif", fontSize: 17, color: '#0f1f17', display: 'flex', alignItems: 'center', gap: 8 }}>
              <Activity size={16} color="#1a5f3a" /> Recent Activity
            </h3>
        ...
        <div style={{
          ...
          <h3 style={{ fontFamily: "'DM Serif Display', serif", fontSize: 17, color: '#0f1f17', marginBottom: 20, display: 'flex', alignItems: 'center', gap: 8 }}>
            <Zap size={16} color="#1a5f3a" /> System Status
          </h3>
