...
import { User, Microscope, Trash2 } from 'lucide-react';
...
const UserManagement = ({ admin }) => {
  ...
  const totalCommunityCount = allUsers.filter((u) => u.userType === 'community').length;
  const totalResearcherCount = allUsers.filter((u) => u.userType === 'researcher').length;
  ...
  const filteredUsers = users.filter((u) => {
    ...
    return (
      ...
      (u.userType || '').toLowerCase().includes(normalizedSearch)
    );
  });
    ...
    <div className="um-root">
      ...
      <div className="um-filter-tabs">
        {[
          ...
          { key: 'community', label: 'Community', count: totalCommunityCount },
          { key: 'researcher', label: 'Researchers', count: totalResearcherCount },
        ].map((tab) => (
          <button
            key={tab.key}
      ...
      {selectedUser && userDetails ? (
            ...
            <>
              ...
              <div className="um-profile-card">
                <div className="um-profile-avatar">
                  {userDetails.userType === 'researcher' ? <Microscope size={36} /> : <User size={36} />}
                </div>
        ...
        <>
          {filteredUsers.length === 0 ? (
            <div className="um-empty">
              <div className="um-empty-icon">👥</div>
              <p>No users found for current search/filter.</p>
            </div>
          ) : (
            <div className="um-users-grid">
              {filteredUsers.map((user) => (
                <div
                  key={user._id}
                  className="um-user-card um-clickable-card"
                  role="button"
                  tabIndex={0}
                  onClick={() => fetchUserDetails(user._id)}
                  onKeyDown={(e) => {
                    if (e.key === 'Enter' || e.key === ' ') fetchUserDetails(user._id);
                  }}
                >
                  <div className="um-user-card-accent" data-type={user.userType} />
                  <div className="um-user-card-body">
                    <div className="um-user-avatar">
                      {user.userType === 'researcher' ? <Microscope size={26} /> : <User size={26} />}
                    </div>
