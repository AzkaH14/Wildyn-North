...
import { notificationEmitter } from '../../contexts/GlobalNotificationContext';

// ─── Config ──────────────────────────────────────────────────────────────────
const API_URL = 'http://192.168.100.2:5000';
const ALERT_RADIUS_KM = 15;
const RAWALPINDI_COORDS = { latitude: 33.5651, longitude: 73.0169 };
const USE_TEST_LOCATION = true; // Testing ke liye consistent location set
const EMITTED_KEY = 'instant_alert_emitted_ids_v1';
const EMITTED_TTL_MS = 5 * 60 * 1000; // 5 minutes: re-test ke liye enough
...
const InstantAlertScreen = () => {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      const storedEnabled = await loadToggleState();
      await initScreen(storedEnabled);
    };
  ...
  const emitCriticalAlerts = async (enriched, enabledForEmit) => {
    if (!enabledForEmit) return;
    if (!Array.isArray(enriched) || enriched.length === 0) return;

    try {
      const storedIds = await AsyncStorage.getItem(EMITTED_KEY);
      const now = Date.now();
      const knownEntries = storedIds ? JSON.parse(storedIds) : [];
      const entriesArr = Array.isArray(knownEntries) ? knownEntries : [];
      const validEntries = entriesArr
        .filter(e => e && typeof e.id === 'string' && typeof e.ts === 'number' && now - e.ts < EMITTED_TTL_MS);
      const knownSet = new Set(validEntries.map(e => e.id));

      // Testing: agar backend severity critical/high ke bina bhej de,
      // to bhi user ko "notification receive" dikhna chahiye.
      // Max 5 naye alerts emit karte hain to avoid spamming.
      const candidates = enriched.slice(0, 5);
      const toEmit = candidates.filter(a => !knownSet.has(a.id));
      if (!toEmit.length) return;

      toEmit.forEach((a) => {
        const isInjured = a.health === 'Injured' || a.health === 'Sick';
        notificationEmitter.emit({
          _id: a.id,
          healthStatus: isInjured ? 'Injured' : 'Normal',
          specieName: a.species,
          username: a.reportedBy,
      ...
      const nextEntries = [
        ...toEmit.map(a => ({ id: a.id, ts: now })),
        ...validEntries,
      ].slice(0, 100);
      await AsyncStorage.setItem(EMITTED_KEY, JSON.stringify(nextEntries));
    } catch (e) {
      console.warn('emitCriticalAlerts error:', e);
    }
  };

  const initScreen = async (enabledForEmit = alertsEnabled) => {
    setLocLoading(true);
    ...
    try {
      if (USE_TEST_LOCATION) {
        coords = { latitude: RAWALPINDI_COORDS.latitude, longitude: RAWALPINDI_COORDS.longitude };
        setLocation(coords);
      } else {
    ...
    // ── Step 2: Fetch alerts ──────────────────────────────────────────────
    try {
      const lat = coords?.latitude ?? RAWALPINDI_COORDS.latitude;
      const lon = coords?.longitude ?? RAWALPINDI_COORDS.longitude;
      ...
      setAlerts(enriched);
      // UX: badge ko “test-friendly” banane ke liye hardcoded critical severity par depend na rakhein.
      // Scan results se up-to-5 ko candidates consider karte hain.
      setNotifCount(Math.min(5, enriched.length));

      // Testing ke liye: scan results me se alerts pe instant in-app notification banner
      await emitCriticalAlerts(enriched, enabledForEmit);
    } catch (fetchErr) {
