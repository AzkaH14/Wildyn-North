...
// Researcher status badge config — all gray
const getResearcherStatusConfig = (status) => {
  switch (status) {
    case 'verified':
      return { label: 'Verified', icon: 'checkmark-circle-outline' };
    case 'duplicate':
      return { label: 'Duplicate', icon: 'git-merge-outline' };
    case 'under_review':
      return { label: 'Under Review', icon: 'time-outline' };
    default:
      return null;
  }
};
          ...
          {loading && reports.length === 0 ? (
            ...
            <FlatList
              ...
              renderItem={({ item }) => {
                ...
                const researcherStatusConfig = getResearcherStatusConfig(item.researcherStatus);
                  ...
                  <View style={styles.card}>
                    ...
                    <View style={styles.cardBody}>
                      ...
                      <View style={styles.reporterRow}>
                        ...
                        {researcherStatusConfig && (
                          <View style={styles.inlineStatusBadge}>
                            <Ionicons name={researcherStatusConfig.icon} size={13} color="#888" />
                            <Text style={styles.inlineStatusText}>{researcherStatusConfig.label}</Text>
                          </View>
                        )}
                      </View>
...
const styles = StyleSheet.create({
  ...
  // Researcher inline status (right of reporter row)
  inlineStatusBadge: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 4,
