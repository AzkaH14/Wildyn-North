/**
 * ResearcherNotifications.js  ── UPDATED VERSION
 *
 * KEY CHANGES:
 * ────────────────────────────────────────────────────────────────────────
 *  Real notifications SharedNotificationService se aati hain.
 *  "Pending" aur "Viewed" sections + pull-to-refresh.
 *  High priority (Injured) alerts prominently highlighted.
 * ────────────────────────────────────────────────────────────────────────
 */
...
const TINT = '#6A0DAD'; // Researcher purple theme
...
export default function ResearcherNotificationsScreen() {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      console.error('[ResearcherNotifs] loadData error:', e);
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const renderCard = (item, isPending) => {
    ...
    return (
      ...
      >
        ...
        <View style={[
          styles.cardIconBox,
          { backgroundColor: isHigh ? '#FFEBEE' : '#F3E8FF' },
        ]}>
          <Ionicons
            name={isHigh ? 'alert-circle' : 'search'}
            size={20}
        ...
        <View style={styles.cardBody}>
          <View style={styles.cardTitleRow}>
            {isHigh && isPending && (
              <View style={styles.highPill}>
                <Text style={styles.highPillTxt}>URGENT</Text>
              </View>
            )}
            <Text
              style={[
          ...
          <View style={styles.cardFooter}>
            ...
            <View style={[
              styles.statusPill,
              isPending ? (isHigh ? styles.pillUrgent : styles.pillNew) : styles.pillReviewed,
            ]}>
              <Text style={[
                styles.statusPillTxt,
                isPending ? (isHigh ? styles.pillUrgentTxt : styles.pillNewTxt) : styles.pillReviewedTxt,
              ]}>
                {isPending ? (isHigh ? 'Urgent' : 'New') : 'Reviewed'}
              </Text>
            </View>
          </View>
        </View>
    ...
    return (
      <SafeAreaView style={styles.safe}>
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle}>Researcher Notifications</Text>
          <View style={{ width: 24 }} />
        </View>
      ...
      <View style={styles.header}>
        ...
        {pending.length > 0 ? (
          <TouchableOpacity onPress={markAllRead} hitSlop={{ top:8, bottom:8, left:8, right:8 }}>
            <Ionicons name="checkmark-done" size={22} color="#CE93D8" />
          </TouchableOpacity>
        ) : (
          <View style={{ width: 24 }} />
        )}
            ...
            {notifications.length === 0 && (
              <View style={styles.emptyBox}>
                ...
                <Text style={styles.emptyDesc}>
                  Community user jab report upload karega, yahan alert aayega.
                </Text>
              </View>
            )}
            ...
            {pending.length > 0 && (
              <View style={styles.section}>
                <View style={styles.sectionHeader}>
                  <View style={[styles.sectionDot, { backgroundColor: '#B71C1C' }]} />
            ...
            {viewed.length > 0 && (
              <View style={styles.section}>
                <View style={styles.sectionHeader}>
                  ...
                  <Text style={[styles.sectionTitle, { color: '#aaa' }]}>Reviewed</Text>
                </View>
...
const styles = StyleSheet.create({
  ...
  unreadDot: { width: 8, height: 8, borderRadius: 4, backgroundColor: '#6A0DAD', flexShrink: 0 },
  ...
  pillUrgent:      { backgroundColor: '#FEF3F2' },
  pillNew:         { backgroundColor: '#EFF8FF' },
  pillReviewed:    { backgroundColor: '#F5F5F5' },
  statusPillTxt:   { fontSize: 10, fontWeight: '700' },
  pillUrgentTxt:   { color: '#B42318' },
  pillNewTxt:      { color: TINT      },
  pillReviewedTxt: { color: '#aaa'    },
});
