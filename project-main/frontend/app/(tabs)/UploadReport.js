...
const UploadReport = () => {
  ...
  const [isAnimalImage, setIsAnimalImage]     = useState(false);
  ...
  const validateAnimalImage = async (dataUri) => {
    try {
      setIsValidatingImage(true);
      setIsAnimalImage(false);
      setAnimalSuggestion('');
      const response = await fetch(`${API_URL}/api/reports/validate-animal`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (!response.ok) {
        Alert.alert('Validation Error', result.message || 'Failed to validate image.');
        return false;
      }
      if (!result.isAnimal) {
        Alert.alert('Invalid Image', result.message || 'No animal detected. Please choose another image.');
        return false;
      }
      const topPredictionLabel = typeof result.topPrediction?.label === 'string'
        ? result.topPrediction.label.trim()
        : '';
      setIsAnimalImage(true);
      setAnimalSuggestion(topPredictionLabel);
      return true;
    } catch (error) {
      console.error('Animal validation error:', error);
      Alert.alert('Connection Error', 'Could not validate image with server.');
      return false;
    } finally {
  ...
  const processSelectedImage = async (selected) => {
    if (!checkImageQuality(selected)) return;
    ...
    const dataUri = `data:${getImageMimeType(selected)};base64,${selected.base64}`;
    const validAnimal = await validateAnimalImage(dataUri);
    if (!validAnimal) {
      setImage(null);
      setImageBase64(null);
      return;
    ...
    Alert.alert('Image Accepted', 'Animal detected. Image is ready to upload.');
  };
  ...
  const identifySpecie = async () => {
    ...
    if (!isAnimalImage) {
      Alert.alert('Animal Not Validated', 'AI Identify works only after an animal image is validated.');
      return;
    }
    if (!animalSuggestion) {
      Alert.alert('No Suggestion Available', 'Could not fetch an animal name suggestion. Try another image.');
      return;
    }
    setSpecieName(animalSuggestion);
    Alert.alert('AI Suggestion', `Suggested animal: ${animalSuggestion}`);
  };

  const handleUpload = async () => {
    if (!imageBase64 || !specieName || !selectedHealth) {
      Alert.alert('Incomplete', 'Please fill all required fields.');
      return;
    }
    if (!isAnimalImage) {
      Alert.alert('Invalid Image', 'Please choose an image that contains an animal.');
      return;
    }
      ...
      <ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.sectionCard}>
          ...
          <View style={styles.specieRow}>
            ...
            />
            <TouchableOpacity
              onPress={identifySpecie}
              style={[styles.aiButton, (!isAnimalImage || isValidatingImage || uploading) && styles.disabledButton]}
              disabled={!isAnimalImage || isValidatingImage || uploading}
            >
        ...
        {/* ══ 6. Upload Button ══ */}
        <TouchableOpacity 
          style={[styles.uploadButton, (uploading || isValidatingImage) && styles.disabledButton]} 
          onPress={handleUpload}
          disabled={uploading || isValidatingImage}
        >
          <Ionicons name="cloud-upload" size={20} color="#000" />
          <Text style={styles.uploadButtonText}>  {isValidatingImage ? 'VALIDATING IMAGE...' : uploading ? 'UPLOADING...' : 'Submit Wildlife Report'}</Text>
        </TouchableOpacity>
