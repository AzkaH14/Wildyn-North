...
import { notificationEmitter } from './GlobalNotificationContext';
import {
...
export const AlertProvider = ({ children }) => {
  ...
  // ✅ Polling se aane wale events ko banner mein convert karo
  // Yeh sab logged-in devices par kaam karta hai — koi provider issue nahi
  useEffect(() => {
    const unsub = notificationEmitter.on((report) => {
      if (report.healthStatus === 'Injured') {
        addHighAlert(
          '🚨 INJURED ANIMAL REPORTED',
          `${report.specieName} needs immediate attention!\nLocation: ${
        ...
        addNormalAlert(
          ...
          `${report.specieName} spotted by ${report.username}`,
          report._id
        );
      }
    });
